...
class Main {
	...
	*/
	public static function _defSubURi() {
		$c = str_ireplace($_SERVER['DOCUMENT_ROOT'], '', trim($_SERVER['SCRIPT_NAME'], '/'));
		$c = str_ireplace(trim($_SERVER['SCRIPT_NAME'], '/'), '', $c);
		$f = '';
		for ($i = 0; $i <= (strlen($_SERVER['REQUEST_URI']) - 1); $i++) {
			if (!isset($c[$i]) || !isset($_SERVER['REQUEST_URI'][$i])) break;
			if ($_SERVER['REQUEST_URI'][$i] == $c[$i]) $f.=$c[$i];
			else break;
		}
		$sub_uri = trim($f, '/\\');
		if ($sub_uri != '') return $sub_uri . '/';
		return '';
	}
