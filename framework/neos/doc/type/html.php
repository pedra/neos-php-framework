...
class Html
	...
	*/
	function produce($get = false){
		...
		foreach(\Doc\Factory::this()->varViews as $nomeView=>$valView){
			...
			$varq = $valView['local'] . \Doc\Factory::this()->varType . DS . $valView['file'] . EXTVW;
			
			//pegando a view do template - se existir
			if(\_cfg::this()->template != ''){
				if( file_exists(\_cfg::this()->template_path . \_cfg::this()->template . DS . 'engine' . DS . 'views' . DS . $valView['file'] . EXTVW)) {
					$varq = \_cfg::this()->template_path . \_cfg::this()->template . DS . 'engine' . DS . 'views' . DS . $valView['file'] . EXTVW;
				}
			}
			...
			while($ret = $this->_pegatag($neosarquivo, $ponteiro)){
				...
				if (isset($ret['-tipo-'])){
					...
					elseif ($ret['-tipo-'] == 'charset'){$vartemp = \_cfg::this()->charset; }
					else {
	...
	*/
	function __script($ret){
		...
		if(isset($ret['rel'])) $url = ( strpos($ret['rel'], 'template') !== false ) ? URL . _cfg('template_url') . \_cfg::this()->template . '/' : $url;
	...
	*/
	 ...
	 * @return string|html
	*/
	function __view($ret, $nomeView){//exit(_pt($ret));
		if(isset($ret['name'])){
			if(file_exists(APP_VIEW  . \Doc\Factory::this()->varType . DS . $ret['name'] . EXTVW)){
			//if(file_exists(APP_VIEW . $ret['name'].'.html')){
				//return file_get_contents(APP_VIEW .$ret['name'].'.html');
				return file_get_contents(APP_VIEW  . \Doc\Factory::this()->varType . DS . $ret['name'] . EXTVW);
			}
		}
	}
		...
		if(isset($r['name'])){
			if($r['name']=='head'){
				...
				if(is_array($this->_html)){
					return $this->_html['doctypes'][\_cfg::this()->html->doctype] ."\n"
							.$this->_html['htmltypes'][\_cfg::this()->html->type] ."\n"
							.'<head>'."\n"
							.'<meta http-equiv="Content-Type" content="text/html; charset='.\_cfg::this()->charset.'" />'."\n"
