	...
	function produce($get = false){
		...
		foreach(Factory::this()->varViews as $nomeView=>$valView){
			...
			if(\_cfg::this()->template != ''){
				if( file_exists(\_cfg::this()->template_path . \_cfg::this()->template . DS . 'engine' . DS . 'views' . DS . $valView . EXTVW)) {
					$varq = \_cfg::this()->template_path . \_cfg::this()->template . DS . 'engine' . DS . 'views' . DS . $valView . EXTVW;
				}
			}
