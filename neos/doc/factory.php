...
class Factory
	...
	function __construct(){
		...
		$this->varPathCss = \_cfg::this()->pathCss;
		$this->varUrlCss = \_cfg::this()->urlCss;
		
		//pegando os PATHs para JavaScript
		$this->varPathJs = \_cfg::this()->pathJs;
		$this->varUrlJs = \_cfg::this()->urlJs;
		
		$this->varCompress = \_cfg::this()->out_compress;
	}
	...
	*/
	static function set($view='index', $nome='', $local = APP_VIEW){
	...
	*/
	function produce($content = ''){
		...
		//limpando o buffer...
		if(\_cfg::this()->out_restricted) ob_clean();		
		
		if (\_cfg::this()->template != '') {
		//Se deve usar um template...
			if (file_exists(\_cfg::this()->template_path . \_cfg::this()->template . DS . 'engine' . DS . 'template.php')) {
				include \_cfg::this()->template_path . \_cfg::this()->template . DS . 'engine' . DS . 'template.php';
	...
	*/
	function outBuffer($out,$md){	
		...
		$body .= (strpos(\_cfg::this()->status,'display') !== false) ? $this->statusBar((strpos(\_cfg::this()->status,'extended') !== false) ? true : false) : '';		
...
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
	...
	*/
	...
	function statusBar($extended = true){
		$sb = ($extended) ? '<script type="text/javascript">var neos_="none";function neostatus(){var dv=document.getElementById("neostatus").style;if(neos_=="none"){neos_="block";dv.top="10px";dv.width="430px"}else{neos_="none";dv.top="";dv.width=""};document.getElementById("neostatustable").style.display=neos_}</script>' : '';
		$sb .= '<style>#neostatus{position:fixed;bottom:10px;right:10px;z-index:200;background:#000;background-color:rgba(0,0,0,0.7);border:2px solid #FFF;border-radius:10px;box-shadow:0 0 80px #555}#neostatus p{padding:0 5px 1px 5px;text-align:right;}#neostatus *,#neostatus{font-size:10px;color:#FFF;margin:0;padding:2px;font-family:Helvetica,Tahoma,monospace,\'Courier New\',Courier,serif;margin:0;overflow:auto}'.(($extended)?'#neostatus{cursor:pointer}#neostatustable{display:none;margin:10px auto;padding:0}#neostatustable tr td{background:transparent !important;padding:2px;margin:0;word-break:break-all;border-bottom:1px solid #666}#neostatustable tr th{font-size:12px;font-weight:bold}#neostatustable pre{white-space:pre-wrap}.neostatuslg td{border-bottom:1px dashed #999}.neostatusright{text-align:right;word-break:normal !important}':'').'</style><!--[if lt IE 6]><style>#neostatus{position:absolute}</style><![endif]--><div id="neostatus"'.(($extended)?' onClick="neostatus()"':'').'>';		
		if($extended){
			...
			$sb .= '<tr><td><b>Total</b> ('.$cf.' arquivos )</td><td class="neostatusright">'.number_format($ct/1000,2,',','.').'&nbsp;kb</td></tr>';
		...
		return $sb.'<p>'.number_format(round(((memory_get_usage()+memory_get_peak_usage())/2000),0),0,',','.').' kb | '.number_format((microtime(true)-NEOS_INIT_TIME)*1000,0,',','.').' ms</p></div>';
	}
